...
class Prelodr {
  ...
  _show(text, cb, ref) {
    ...
    setTimeout(() => {
      const cls = `${ref.options.prefixClass} ${ref.options.prefixClass}-in`;
  ...
  _queueWalk(ref) {
    const that = ref || this
    const one = that.queu.first();

    if (one && one.is === 'in') {
      that.isShown = true;

      one.fn(() => {
        const next = that.queu.first();
        that.isStart = false;
  ...
  _prepOut(cb, that) {
    ...
    setTimeout(() => {
      ...
      if (el) {
        ...
        const cls = `${that.options.prefixClass} ${that.options.prefixClass}-out`;
